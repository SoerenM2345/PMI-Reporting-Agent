...
## 3. No machine-readable ground-truth key — DONE as of v1.0
`../ground_truth.json` (the 6 planted conflicts) and `../error_key.json` (the 10 injected
errors), both exported from `case.py`/`error_key.py` by `export_ground_truth.py` rather
than hand-retyped. Full per-entity `stated_in` provenance for the entire entity set
(needed to score extraction recall per-entity, not just the 16 findings) is still open —
see `ground_truth.json`'s own `known_gaps` field.
...
- Flawed copy with 10 injected errors and an error key workbook (`../with_errors/`,
  `../00_ERROR_KEY.xlsx`)
- Machine-readable ground truth: `../ground_truth.json`, `../error_key.json`
- Corpus moved into a versioned, manifest-integrity-checked home: `v1.0/`
  (`MANIFEST.sha256`, `tests/test_corpus_dellemc.py`)
